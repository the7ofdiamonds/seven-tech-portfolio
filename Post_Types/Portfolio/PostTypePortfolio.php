...
class PostTypePortfolio
{
    ...
    function get_project()
    {
        if (!empty($_GET['post'])) {
            ...
            if ($this->post_id) {
                $portfolio_project = new PortfolioProject;
                $this->project = $portfolio_project->getPortfolioProject($this->post_id);
            }
        }
    }
...
<?php }
    ...
    function save_project($post_id)
    {
        ...
        $project = [
            ...
            'project_status' => $portfolio_project->getProjectStatus($post_id),
        ...
        $existing_project = $portfolio_project->getPortfolioProject($post->ID);

        if (empty($existing_project)) {
            return $portfolio_project->createPortfolioProject($project);
        }

        return $portfolio_project->updatePortfolioProject($project);
    }
}
