...
class ProjectTags
{
    ...
    public function __construct()
    {
        ...
        add_filter('manage_edit-project_tags_columns', [$this, 'edit_columns']);
        add_action('manage_project_tags_custom_column', [$this, 'manage_columns'], 10, 3);
        add_action('project_tags_add_form_fields', [$this, 'add_fields']);
        add_action('project_tags_edit_form_fields', [$this, 'edit_fields'], 10, 2);
        add_action('created_project_tags', [$this, 'save_fields']);
        add_action('edited_project_tags', [$this, 'save_fields']);
    }
    ...
    public function getProjectTags($post_type)
    {
        try {
            ...
            foreach ($project_tags as $projectTag) {
                $faIcon = get_term_meta($projectTag->term_id, 'fa_icon', true);
                $iconURL = get_term_meta($projectTag->term_id, 'icon_url', true);

                $projectTags[] = [
                    'id' => $projectTag->term_id,
